...
def draw_city_background():
    ...
    for building in buildings[:]:
        ...
        ground_overlap = 10  # Increased overlap to hide the lower border
        ...
        # Draw a doorway on the building
        door_width = 10
        door_height = 20
        door_x = screen_x + screen_width // 2 - door_width // 2
        door_y = screen_y - door_height
        # Draw a doorway on the building
        door_width = 10
        door_height = 20
        door_x = screen_x + screen_width // 2 - door_width // 2
        door_y = screen_y + screen_height - door_height - 5  # Adjust door position to be at the bottom
...
def draw_obstacles():
    for obstacle in obstacles:
        if obstacle['is_boss']:
            ...
            # Draw boss with airplane shape
            screen_x, screen_y = project_3d_to_2d(obstacle['x'] - WIDTH // 2, obstacle['y'] - HEIGHT // 2, obstacle['z'])
            size = int(obstacle['size'] / obstacle['z'])
            # Main body of the boss airplane
            display.triangle(screen_x, screen_y - size, screen_x - size // 2, screen_y + size, screen_x + size // 2, screen_y + size)
            # Wings of the boss airplane
            display.triangle(screen_x - size, screen_y, screen_x - size // 4, screen_y + size, screen_x, screen_y)
            display.triangle(screen_x + size, screen_y, screen_x + size // 4, screen_y + size, screen_x, screen_y)
        else:
            display.set_pen(RED)
            # Draw regular enemy with airplane shape
            screen_x, screen_y = project_3d_to_2d(obstacle['x'] - WIDTH // 2, obstacle['y'] - HEIGHT // 2, obstacle['z'])
            size = int(obstacle['size'] / obstacle['z'])
...
def draw_city_background():
    ...
    # Limit the number of buildings to 3
    visible_buildings = buildings[:3]
    ...
    for building in visible_buildings:
        ...
        if building["z"] > 9.0:
            # Draw the building
            display.set_pen(building["color"])
            display.rectangle(screen_x, screen_y, screen_width, screen_height)
    
    # Only spawn new buildings if there are fewer than 3
    if len(buildings) < 3 and random.randint(0, 10) == 0:
        x = random.randint(0, WIDTH)
        z = random.uniform(10.0, 20.0)
        width = random.randint(20, 60)
        height = random.randint(50, 150)  # Allow buildings to be taller
